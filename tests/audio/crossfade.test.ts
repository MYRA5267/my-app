...
/** play() подтверждает старт асинхронно; ждём microtask перед движением rAF. */
async function loadAudio(url: string) {
  await act(async () => {
    apiRef.current!.load(url);
...
describe("useAudio: кроссфейд при повторном прерывании (baг 'volume вне [0,1]')", () => {
  it("не бросает и не выставляет .volume вне [0,1] при быстрой повторной смене трека посреди активного кроссфейда", async () => {
    // Любое исключение из этих вызовов само провалит тест.
    await loadAudio("track-1.mp3"); // первая загрузка — без кроссфейда (ничего ещё не играло)
    act(() => { apiRef.current!.setVolume(1.0000000005); }); // имитация float-дрейфа громкости "сверху" из UI
    await loadAudio("track-2.mp3"); // trackом-1 уже "играет" — запускается кроссфейд
    act(() => { advanceFrame(FADE_MS * 0.5); }); // долистали кроссфейд до середины (k≈0.5)
    await loadAudio("track-3.mp3"); // прерываем ДО завершения предыдущего кроссфейда
    act(() => { advanceFrame(FADE_MS * 0.5); });
    await loadAudio("track-4.mp3"); // прерываем ещё раз
  ...
  it("после прерывания кроссфейда старый (фейд-аут) элемент поставлен на паузу и его src очищен", async () => {
    await loadAudio("track-1.mp3"); // pair[0] играет track-1
    await loadAudio("track-2.mp3"); // кроссфейд: pair[0] фейд-аут, pair[1] фейд-ин (track-2)
  ...
  it("после серии из 5 быстрых next подряд ровно один аудиоэлемент играет актуальный трек", async () => {
    const urls = ["t1.mp3", "t2.mp3", "t3.mp3", "t4.mp3", "t5.mp3"];
    for (const url of urls) {
      await loadAudio(url); // каждый следующий load() прерывает предыдущий кроссфейд, не дожидаясь его конца
    }
