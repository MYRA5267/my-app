...
-- ============================================================================
-- 8.1. creator_verification_requests — реальные заявки артистов на проверку
-- ============================================================================
-- Снимок критериев с клиента нужен только для удобства очереди. Решение всё
-- равно принимает администратор после проверки серверных релизов и метрик.
...
drop policy if exists "creator_verification_requests_select_participant_or_admin" on public.creator_verification_requests;
...
drop policy if exists "creator_verification_requests_insert_own" on public.creator_verification_requests;
create policy "creator_verification_requests_insert_own"
  on public.creator_verification_requests for insert
  with check (user_id = (select auth.uid()) and status = 'pending');

drop policy if exists "creator_verification_requests_update_admin" on public.creator_verification_requests;
